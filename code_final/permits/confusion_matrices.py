...
def eval_coteaching(X_test_orig, X_test, y_test, path1, path2):
    ...
    not_indices = np.ones(len(y_test))
    not_indices[indices] = 0
    not_indices = not_indices.astype(bool)
    # number of true positives
    TP = sum(y_test.numpy()[indices])

    # number of true negatives
    TN = sum(1-y_test.numpy()[not_indices])

    # number of false positives
    # labeled as positive, but true value is 0
    FP = sum(1-y_test.numpy()[indices])

    # number of false negatives
    # labeled as negative, but true value is 1
    FN = sum(y_test.numpy()[not_indices])
...
def main():
    ...
    eval_coteaching(X_test_orig, X_test, y_test, path1 = "data/nn_coteaching1_0pct.pt", path2 = "data/nn_coteaching2_0pct.pt")
