...
nums <- fac.final.hist.insp %>% group_by(REGISTRY_ID) %>%
  summarize(n = n())
#random selection
set.seed(1234)
...
save(random.full, recent.full, file = "full.data.for.models.Rdata")
...
save(train.random, test.random, file = "full.data.random.Rdata")

train.recent <- recent.full[smplmain, ]
test.recent <- recent.full[-smplmain, ] #this is the 20% hold-out
save(train.recent, test.recent, file = "full.data.recent.Rdata")
